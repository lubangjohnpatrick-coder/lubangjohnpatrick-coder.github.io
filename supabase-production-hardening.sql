-- AACE Project Dashboard — production hardening migration
-- Run once in Supabase SQL Editor BEFORE deploying the matching frontend.
-- Safe to re-run: policies/functions/triggers are replaced idempotently.
...
create index if not exists idx_activity_trail_project_created
  on public.activity_trail(project_id, created_at desc);
...
-- --------------------------------------------------------------------------
-- Authorization helpers. These are SECURITY DEFINER so RLS checks do not
-- recursively depend on user_profiles policies.
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- Profiles: ordinary users can read their own profile and may update only
-- non-privileged fields. Account creation/role/status/permission management is
-- admin-only and is performed by the server-side admin-users Edge Function.
-- --------------------------------------------------------------------------
...
-- --------------------------------------------------------------------------
-- Projects: database authorization mirrors the UI permissions. DELETE stays
-- administrator-only because deletion is materially more destructive than edit.
-- --------------------------------------------------------------------------
...
-- Counters are never exposed directly to browser clients.
...
-- --------------------------------------------------------------------------
-- Collision-safe server-side project creation. Project numbers roll over by
-- Manila calendar year and are allocated atomically inside PostgreSQL.
-- --------------------------------------------------------------------------
...
begin
  ...
  if not public.has_project_permission('add') then
    raise exception 'Add-project permission required';
  end if;
...
-- --------------------------------------------------------------------------
-- Immutable audit trail. Database triggers capture every project insert/update/
-- delete even when a change is made outside this browser application.
-- --------------------------------------------------------------------------
...
begin
  if new.source <> 'manual' then
    raise exception 'Only manual audit notes may be inserted directly';
  end if;
  if auth.uid() is null or not public.has_project_permission('edit') then
    raise exception 'Edit permission required';
  end if;
  new.actor_id := auth.uid();
  select coalesce(up.username, 'unknown') into new.actor_username
...
create trigger trg_activity_manual_actor
before insert on public.activity_trail
for each row
when (new.source = 'manual')
execute function public.stamp_manual_activity_actor();
...
begin
  if v_actor is not null then
    select coalesce(up.username, 'unknown') into v_username
  ...
  if tg_op = 'INSERT' then
    ...
    insert into public.activity_trail(project_id, actor_id, actor_username, action, note, source, before_data, after_data)
      values (v_project_id, v_actor, v_username, 'project_created', 'Project created.', 'system', null, new.data);
    return new;
    ...
    v_project_id := old.id;
    insert into public.activity_trail(project_id, actor_id, actor_username, action, note, source, before_data, after_data)
      values (v_project_id, v_actor, v_username, 'project_deleted', 'Project deleted.', 'system', old.data, null);
    return old;
  end if;
end;
$$;
...
-- Users with view permission can read the audit trail; editors may add manual
-- notes. No browser role can update/delete audit history (append-only).
drop policy if exists "activity_select" on public.activity_trail;
create policy "activity_select" on public.activity_trail
  for select to authenticated
  using (public.has_project_permission('view'));

drop policy if exists "activity_insert_manual" on public.activity_trail;
create policy "activity_insert_manual" on public.activity_trail
  for insert to authenticated
...
-- Retire the earlier browser-callable Auth deletion RPC. Auth administration is
-- now handled by a Supabase Edge Function using the service-role key server-side.
drop function if exists public.admin_delete_dashboard_user(uuid);
...
-- IMPORTANT FIRST-ADMIN BOOTSTRAP
-- This migration intentionally does NOT allow self-promotion. If starting from a
-- brand-new database, create the first Auth user in Supabase Authentication, then
-- insert its user_profiles row once from the SQL Editor as a trusted operator.
